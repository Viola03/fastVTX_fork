...
full_list_of_decays = []
full_list_of_decays_string = '['
for idx, combination in enumerate(unique_combinations):
    # _Decays.append(f"[ B+ -> {particle_dict[combination[0]]} {particle_dict[combination[1]]} {particle_dict[combination[2]]} ]cc")
...
class GeneralConf(LineBuilder):
    ...
    def _makeB2LLX(
        ...
        _Decays = ["[ B+ -> K+ e+ e- ]cc", "[ B+ -> K+ mu+ mu- ]cc", "[ B+ -> K+ K+ K- ]cc", "[ B+ -> pi+ pi+ pi- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?
        # _Decays = ["[ B+ -> K+ e+ e- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?
        # _Decays = ["[ B+ -> K+ K+ K- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?
        
        # _Decays = ["[ B+ -> pi+ e+ e- ]cc", "[ B+ -> pi+ mu+ mu- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?
        _Decays = ["[ B+ -> K+ e+ e- ]cc", "[ B+ -> K+ mu+ mu- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?


        # THIS MATCHED THE ABOVE - is it because StdAllNoPIDsKaons and StdAllNoPIDsPions have basically 100% overlap - DEBUNKED
        # _Decays = ["[ B+ -> K+ e+ e- ]cc", "[ B+ -> K+ mu+ mu- ]cc", "[ B+ -> K+ K+ K- ]cc", "[ B+ -> pi+ pi+ pi- ]cc"] # PID cuts in StdLoose are wide enough to avoid all patterns and avoid repeated candidates?
...
#  | *AllStreams/Phys/StdAllNoPIDsElectrons/Particles                                                  |       47 |     2.0+-0.1    | 0.5     |    1 | 4      |
#  | *AllStreams/Phys/StdAllNoPIDsKaons/Particles                                                      |      134 |     3.2+-0.3    | 2.0     |    1 | 9      |
#  | *AllStreams/Phys/StdAllNoPIDsMuons/Particles                                                      |      102 |     2.5+-0.1    | 0.9     |    1 | 5      |
#  | *AllStreams/Phys/StdAllNoPIDsPions/Particles                                                      |      174 |     3.9+-0.3    | 2.2     |    1 | 10     |
#  | *AllStreams/Phys/StdAllNoPIDsProtons/Particles  


# path_to_look = "Phys/" + cfg["stripping_line"] + "/Particles"
# path_to_look = "pPhys/Particles"
# path_to_look = "MC/Particles"
