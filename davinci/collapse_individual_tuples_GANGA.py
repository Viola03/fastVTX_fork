...
if use_PFNs:
    ...
    root = tree.getroot()
    pfns = []
    for pfn in root.findall('.//pfn'):
    # for pfn in root.findall('.//lfn'):
        pfns.append(pfn.get('name'))
    print(pfns)
...
if use_PFNs:
    for file_idx, pfn_i in enumerate(pfns):
        ...
        file_trimmed = file.split('/')[-1][:-5]+'_trimmed.root'
        file_merged = file.split('/')[-1][:-5]+'_merged.root'
