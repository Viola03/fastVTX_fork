...
events[f"B_plus_P"], events[f"B_plus_PT"] = compute_B_mom(events, true_vars=False)
events[f"missing_B_plus_P"], events[f"missing_B_plus_PT"] = compute_miss_mom(
...
events["IP_B_plus"] = compute_impactParameter(events, true_vars=False)
for particle in ["K_Kst", "e_plus", "e_minus"]:
    events[f"IP_{particle}"] = compute_impactParameter_i(
        events, f"{particle}", true_vars=False
    )
events["FD_B_plus"] = compute_flightDistance(events, true_vars=False)
events["DIRA_B_plus"] = compute_DIRA(events, true_vars=False)
