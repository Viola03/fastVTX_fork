...
def variable_plotter(pdf, variable, analyserA, analyserB, cut, labelA, labelB, title=None, log=False):
    ...
    alexPlot.plot_data([analyserA.data.query(cut).eval(variable), analyserB.data.query(cut).eval(variable)], density=True, also_plot_hist=True, bins=75, color=['tab:blue','tab:orange'], label=[labelA, labelB], only_canvas=True, pulls=True, log=log)
...
def plot_efficiency_as_a_function_of_variable(pdf, tuple_A, tuple_B, label_A, label_B, variable, cut, range_array, title, xlabel, signal, return_values=False):
        ...
        alexPlot.plot_points(x, y=[eff_A, eff_B], yerr=[effErr_A, effErr_B], density=True, also_plot_hist=True, color=['tab:blue','tab:orange'], label=[label_A, label_B], only_canvas=True, pulls=True)
        ...
        alexPlot.plot_points(x, y=[hist_post_A[0]/np.sum(hist_post_A[0]), hist_post_B[0]/np.sum(hist_post_B[0])], yerr=[np.sqrt(hist_post_A[0])/np.sum(hist_post_A[0]), np.sqrt(hist_post_B[0])/np.sum(hist_post_B[0])], density=True, also_plot_hist=True, color=['tab:blue','tab:orange'], label=[label_A, label_B], only_canvas=True, pulls=True)
        plt.ylim(ymin=0.)
