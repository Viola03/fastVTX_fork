...
myApp.options = ["./davinci_general_mcmatch_intermediates_withChargeCounters.py"]
...
# job.splitter = GenericSplitter(attribute = 'inputfiles', values = [[i, LocalFile('run_postprocess_checker.py'), LocalFile('collapse_individual_tuples_GANGA.py')] for idx, i in enumerate(comp_dataset[:3])])
...
# # job.inputfiles = [LocalFile("./run_postprocess_checker.py"), LocalFile("./collapse_individual_tuples_GANGA.py")]

# # cc = CustomChecker(module='run_postprocess_checker.py', checkMaster = False)
# cc = CustomChecker(module='run_postprocess_checker.py', checkSubjobs = True, checkMaster = False)
# # cc.module = "./run_postprocess_checker.py"
# # job.postprocessors.append(cc)
# job.postprocessors = [ cc ]
